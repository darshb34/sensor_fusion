...
def merge_second_batch(batch_list):
    ...
    for key, elems in example_merged.items():
        if key in [
            ...
            #lol if key == 'voxels':
            #lol    print("VOXELS ", np.concatenate(elems, axis=0).shape)
        elif key == 'metadata':
            ret[key] = elems
                        
####################################################
        # elif key == 'images1':
        #     #lol print("images1 ", np.concatenate(elems, axis=0).shape)

        #     # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@", len(elems),np.concatenate(elems, axis=0).shape )
        #     ret[key] = np.concatenate(elems, axis=0)
            ...
            # print("######################################",key, np.concatenate(elems,axis=0).shape)
...
def prep_pointcloud(input_dict,
    ...
    image_bytes = input_dict["cam"]["data"]
    # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@",points.shape)
    image = cv2.imdecode(np.frombuffer(image_bytes, np.uint8), -1)
    # print('dddddddddddddddddddd', image.shape)
    # res_img = cv2.resize(image, dsize=(640, 526))#, interpolation=cv2.INTER_CUBIC) #160*132
    # res_img = cv2.resize(image, dsize=(800, 701))#, interpolation=cv2.INTER_CUBIC) #200*176
    res_img1 = cv2.resize(image, dsize=(160, 132))#, interpolation=cv2.INTER_CUBIC) #160*132
    res_img = rearrange(res_img1, 'h w c -> c w h')

    # print('*******************************',input_dict['lidar'])
    if training:
    ...
    if training:
        ...
        # if multi_gpu ==False and db_sampler ==None:
        ...
        if db_sampler is not None:
            ...
            sampled_dict = db_sampler.sample_all(
                ...
                calib=calib)
            # if multi_gpu ==False and sampled_dict ==None:
            ...
            if sampled_dict is not None:
                ...
                ####################################
#lol                # sampled_images = sampled_dict["images1"]
                #lol print("just got from sample all function: ",sampled_images.shape)
                # print("$$$$$$$$$$$$$$$$$$$$$$$$$ ",input_dict["image"]['image_shape'],"\n",'$$$$$$$$',input_dict["image"],'\n \n')
                # print("@@@@@@@@@@@@@@@@@@@@@@ ",input_dict.keys())
                ######################################
                ...
                # images = np.concatenate([sampled_images, ])
                #######################################################
            # elif sampled_dict['test_case'] is None:
            #     sampled_images = sampled_dict["images1"]
    ...
    #lol print("sending to example variable tensor: ",sampled_images.shape)
    example = {
        'voxels': voxels,
