...
class DataBaseSamplerV2:
    def __init__(self,
        ...
        # f1= open('myConfig.txt','a')
        # f1.write(str(db_infos)+'\n ')
        # f1.write('oooooooooooooooooooooooooooo \n'+str(groups) + '\n \n \n'+'ppppppppppppppppppppppp \n')
    ...
    def sample_all(self,
        ...
        ###################   ADD HERE IMG LOADER
        # print("########################################################")
        # sampled (batch size)= 12; s_points = (1024,)
        if len(sampled) > 0:
            ...
            for info in sampled:
                s_points = np.fromfile(
                    ...
                    dtype=np.float32)
                # f.write(str(pathlib.Path(root_path) / info["path"])+'\n')
                # f.write(str(info)+'\n')
                # img_idx = str(info["image_idx"])
                # while len(img_idx) != 6:
                #     img_idx = '0'+img_idx

                # a = 'training/image_2/' + img_idx + '.png'
                # img = cv2.imread(str(pathlib.Path(root_path) / a))
                # cv2.imwrite(str(pathlib.Path(root_path)/'b4.png'),img)
                # res_img = cv2.resize(img, dsize=(640, 526))#, interpolation=cv2.INTER_CUBIC)
                # cv2.imwrite(str(pathlib.Path(root_path)/'after.png'),res_img)

                # print(a)
                # img = np.fromfile(
                #     str(pathlib.Path(root_path) / a),
                #     dtype=np.float32)
                # print("lolololololololol: ",img.shape)
                # print("lolololololololol     : ",res_img.shape)

                # f.write('s_points_SHAPE_B44:'+str(s_points.shape))    
                # print('num of sampled='+str(num_sampled)+'\n')
                s_points = s_points.reshape([-1, num_point_features])
                # f.write("sampled"+str(sampled)+'\n'+'num of sampled='+str(num_sampled)+'\n'+'s_points_SHAPE:'+str(s_points.shape)+str(s_points)+'\n')
            ...
            # f.write('_______________________________________ \n')
                # f.write("cccccccccccc :" + str(len(s_points_list))+'   '+str(len(sampled))+'\n')

        
            # f.write("s_points_list:shape:: " + str(len(s_points_list))+'\n')
            # f.write("s_points_list:shape_1:: " + str(np.concatenate(s_points_list, axis=0).shape)+'\n')
            # abc = np.concatenate(s_points_list, axis=0)
            # f.write("s_points_list:shape_2:: " + str(abc.shape)+'\n')
            ...
            # print('$$$$$$$$$$$$$$$ s-point list', len(s_points_list),s_points_list[0].shape,s_points_list[1].shape)
            # print('$$$$$$$$$$$$$$$ res img list', len(res_img_list),res_img_list[0].shape,res_img_list[1].shape)


            if random_crop:
                # f.write("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
                s_points_list_new = []
            ...
            ret = {
                ...
                # "images1": np.concatenate(np.zeros(shape=(225,640,3)), axis=0)
                #"images1": np.concatenate(res_img_list, axis=0)#np.zeros(shape=(2254,640,3)),#"hi"################
            }
            # print("!!!!!!!!!!!!!!!!!!", len(res_img_list) ,np.concatenate(res_img_list, axis=0).shape, np.concatenate(s_points_list, axis=0).shape)
            if self._use_group_sampling:
