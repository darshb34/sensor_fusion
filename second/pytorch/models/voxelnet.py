...
from second.pytorch.models import resnet_c
import torchplus
...
class VoxelNet(nn.Module):
    ...
    def network_forward(self, voxels, images, num_points, coors, batch_size):
        ...
        # voxel_features, images_vfe = self.voxel_feature_extractor(voxels, images, num_points,
                                                    #   coors)
        self.end_timer("voxel_feature_extractor")

        self.start_timer("middle forward")
        # spatial_features, images_mfe = self.middle_feature_extractor(
            # voxel_features, images_vfe, coors, batch_size)
        self.end_timer("middle forward")
        self.start_timer("rpn forward")
        # image_features = "a"
        # print("#######",images.shape)
        # print("#######VV ",voxels.shape)
        # f=open("spa.txt",'a')
        # f.write("Spatial Features: "+ str(spatial_features.shape))
        # print("Spatial Features: ", spatial_features.shape)
        '''
        image_rearranged = rearrange(images, 'b h w c -> b c h w')
        image_features = resnet_c.my_forward(image_rearranged)
        image_features = rearrange(image_features, 'b c h w -> b c w h')
        image_features1 = image_features[:,0:128,:,:]
        '''
        # print("Image Features: ", image_features1.shape)

        # combined_features = torch.cat((spatial_features, image_features),1)

        # image_rearranged = rearrange(images_mfe, 'b h w c -> b c w h')
        # combined_features = torch.cat((spatial_features, image_rearranged),1)
        # image_rearranged = rearrange(images, 'b h w c -> b c w h')
        
        # print("combined_features: ", combined_features.shape)

        # preds_dict = self.rpn(image_features1) #image only
        # preds_dict = self.rpn(spatial_features)  #lidar only
        # preds_dict = self.rpn(combined_features)
        preds_dict = self.rpn(images)
        # print("EVOLVE_____________ \n", preds_dict)
        # print("EVOLVE_################### \n", preds_dict.keys())
        # pdb.set_trace()
    ...
    def forward(self, example):
        ...
        # coors: [num_voxels, 4]
        preds_dict = self.network_forward(voxels, images, num_points, coors, batch_size_dev)
        # need to check size.
